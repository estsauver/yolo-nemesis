function  [w_vector,err_tot]= ANN_Yeast( w_vector, params )

%specify the neural network using w_vector

global X_actual2 S_actual2 E_actual2 DisOx2 Volume2 F2 t_new
...
err(3, :) = E - E_actual;
eX=err(1,:);
err_tot = max([ sqrt(sum(sum(err.^2))), sum(sum(err.^2))]);
%err_tot=max([sqrt(sum(eX.^2)), sum(eX.^2)]);
err_tot=1/err_tot;    
